...
function ModificarCiudad() {
  ...
  const [datos, setDatos] = useState({
    idciudad: params.idciudad, // Cambié de idplato a idciudad
    nombre: "",
    pais: "",
    poblacion: "",
  });
  ...
  useEffect(() => {
    async function getCiudadById() {
      let response = await fetch(apiUrl + "/ciudades/" + datos.idciudad);
      if (response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validarDatos()) {
      ...
      try {
        ...
        const response = await fetch(apiUrl + "/ciudades/" + datos.idciudad, {
          method: "PUT", // "PATCH"
          headers: {
